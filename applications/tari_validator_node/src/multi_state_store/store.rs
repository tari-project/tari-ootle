    ...
    pub fn connect(config: &ApplicationConfig) -> Result<Self, anyhow::Error> {
        let state_store = match config.validator_node.database_type {
            DatabaseType::Rocksdb => {
                ...
                let rocksdb_path = state_db_path
                    .as_path()
                    .join("rocksdb")
                    .as_os_str()
                    .to_str()
...
impl<'tx> StateStoreWriteTransaction for ValidatorNodeStateStoreWriteTransaction<'tx> {
    type Addr = PeerAddress;
    ...
    fn blocks_insert(
        &mut self,
        block: &tari_dan_storage::consensus_models::Block,
    ...
    fn blocks_set_flags(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.blocks_set_flags(block_id, is_committed, is_justified)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.blocks_set_flags(block_id, is_committed, is_justified)
            },
        }
    }

    fn block_diffs_insert(
        &mut self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
        changes: &[tari_dan_storage::consensus_models::SubstateChange],
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.block_diffs_insert(block_id, changes)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.block_diffs_insert(block_id, changes)
            },
    ...
    fn quorum_certificates_insert(
        &mut self,
        qc: &tari_dan_storage::consensus_models::QuorumCertificate,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.quorum_certificates_insert(qc)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => write_tx.quorum_certificates_insert(qc),
        }
    }

    fn quorum_certificates_set_shares_processed(
        &mut self,
        qc_id: &tari_dan_storage::consensus_models::QcId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn last_sent_vote_set(
        &mut self,
        last_sent_vote: &tari_dan_storage::consensus_models::LastSentVote,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.last_sent_vote_set(last_sent_vote)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.last_sent_vote_set(last_sent_vote)
            },
    ...
    fn last_voted_set(
        &mut self,
        last_voted: &tari_dan_storage::consensus_models::LastVoted,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn last_executed_set(
        &mut self,
        last_exec: &tari_dan_storage::consensus_models::LastExecuted,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn last_proposed_set(
        &mut self,
        last_proposed: &tari_dan_storage::consensus_models::LastProposed,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.last_proposed_set(last_proposed)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.last_proposed_set(last_proposed)
            },
    ...
    fn last_proposed_unset(
        &mut self,
        last_proposed: &tari_dan_storage::consensus_models::LastProposed,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.last_proposed_unset(last_proposed)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.last_proposed_unset(last_proposed)
            },
    ...
    fn leaf_block_set(
        &mut self,
        leaf_node: &tari_dan_storage::consensus_models::LeafBlock,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn locked_block_set(
        &mut self,
        locked_block: &tari_dan_storage::consensus_models::LockedBlock,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.locked_block_set(locked_block)
            },
    ...
    fn high_qc_set(
        &mut self,
        high_qc: &tari_dan_storage::consensus_models::HighQc,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn foreign_proposals_upsert(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.foreign_proposals_upsert(foreign_proposal, proposed_in_block)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.foreign_proposals_upsert(foreign_proposal, proposed_in_block)
            },
    ...
    fn foreign_proposals_delete(
        &mut self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.foreign_proposals_delete(block_id)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.foreign_proposals_delete(block_id)
            },
    ...
    fn foreign_proposals_delete_in_epoch(
        &mut self,
    ...
    fn foreign_proposals_set_status(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.foreign_proposals_set_status(block_id, status)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.foreign_proposals_set_status(block_id, status)
            },
    ...
    fn foreign_proposals_set_proposed_in(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.foreign_proposals_set_proposed_in(block_id, proposed_in_block)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.foreign_proposals_set_proposed_in(block_id, proposed_in_block)
            },
    ...
    fn foreign_proposals_clear_proposed_in(
        &mut self,
        proposed_in_block: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn transactions_insert(
        &mut self,
        transaction: &tari_dan_storage::consensus_models::TransactionRecord,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn transactions_update(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.transactions_update(transaction)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.transactions_update(transaction)
            },
    ...
    fn transactions_save_all<'a, I: IntoIterator<Item = &'a tari_dan_storage::consensus_models::TransactionRecord>>(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.transactions_save_all(transaction)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.transactions_save_all(transaction)
            },
    ...
    fn transactions_finalize_all<
        'a,
        I: IntoIterator<Item = &'a tari_dan_storage::consensus_models::TransactionPoolRecord>,
    >(
        &mut self,
        block_id: tari_dan_storage::consensus_models::BlockId,
        transaction: I,
    ...
    fn transaction_executions_remove_any_by_block_id(
        &mut self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn transaction_pool_confirm_all_transitions(
        &mut self,
        new_locked_block: &tari_dan_storage::consensus_models::LockedBlock,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.transaction_pool_confirm_all_transitions(new_locked_block)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.transaction_pool_confirm_all_transitions(new_locked_block)
            },
    ...
    fn missing_transactions_remove(
        ...
        Option<(
            tari_dan_storage::consensus_models::Block,
            Vec<tari_dan_storage::consensus_models::ForeignProposal>,
        )>,
        tari_dan_storage::StorageError,
    > {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn foreign_parked_blocks_insert(
        &mut self,
        park_block: &tari_dan_storage::consensus_models::ForeignParkedProposal,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn foreign_parked_blocks_insert_missing_transactions<
        'a,
        I: IntoIterator<Item = &'a tari_transaction::TransactionId>,
    >(
        &mut self,
    ...
    fn votes_insert(
        &mut self,
        vote: &tari_dan_storage::consensus_models::Vote,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn substate_locks_insert_all<
        ...
        I: IntoIterator<
            Item = (
                &'a tari_engine_types::substate::SubstateId,
                &'a Vec<tari_dan_storage::consensus_models::SubstateLock>,
            ),
        >,
    >(
        &mut self,
    ...
    fn substate_locks_remove_any_by_block_id(
        &mut self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_create(
        &mut self,
        substate: &tari_dan_storage::consensus_models::SubstateRecord,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_down(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => write_tx.substates_down(
            ...
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => write_tx.substates_down(
                versioned_substate_id,
                shard,
                epoch,
                destroyed_block_height,
    ...
    fn pending_state_tree_diffs_remove_and_return_by_block(
        ...
        indexmap::IndexMap<
            tari_dan_common_types::shard::Shard,
            Vec<tari_dan_storage::consensus_models::PendingShardStateTreeDiff>,
        >,
        tari_dan_storage::StorageError,
    > {
        match self {
    ...
    fn state_tree_shard_versions_set(
        &mut self,
        shard: tari_dan_common_types::shard::Shard,
        version: Version,
    ) -> Result<(), tari_dan_storage::StorageError> {
    ...
    fn epoch_checkpoint_save(
        &mut self,
        checkpoint: &tari_dan_storage::consensus_models::EpochCheckpoint,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn burnt_utxos_insert(
        &mut self,
        burnt_utxo: &tari_dan_storage::consensus_models::BurntUtxo,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.burnt_utxos_insert(burnt_utxo)
            },
    ...
    fn burnt_utxos_clear_proposed_block(
        &mut self,
        proposed_in_block: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn burnt_utxos_delete(
        ...
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.burnt_utxos_delete(commitment)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => write_tx.burnt_utxos_delete(commitment),
        }
    }

    fn lock_conflicts_insert_all<
        'a,
        I: IntoIterator<
            Item = (
                &'a tari_transaction::TransactionId,
                &'a Vec<tari_dan_storage::consensus_models::LockConflict>,
            ),
        >,
    >(
        &mut self,
    ...
    fn validator_epoch_stats_add_participation_share(
        &mut self,
        qc_id: &tari_dan_storage::consensus_models::QcId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
                write_tx.validator_epoch_stats_add_participation_share(qc_id)
            },
            ValidatorNodeStateStoreWriteTransaction::Sqlite { write_tx, .. } => {
                write_tx.validator_epoch_stats_add_participation_share(qc_id)
            },
    ...
    fn validator_epoch_stats_updates<
        'a,
        I: IntoIterator<Item = tari_dan_storage::consensus_models::ValidatorStatsUpdate<'a>>,
    >(
        &mut self,
        epoch: tari_dan_common_types::Epoch,
        updates: I,
    ...
    fn diagnostics_add_no_vote(
        &mut self,
        block_id: tari_dan_storage::consensus_models::BlockId,
        reason: tari_dan_storage::consensus_models::NoVoteReason,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
    ...
    fn lock_conflicts_remove_by_block_id(
        &mut self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
    ...
    fn evicted_nodes_evict(
        ...
        evicted_in_block: tari_dan_storage::consensus_models::BlockId,
    ) -> Result<(), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreWriteTransaction::Rocksdb { write_tx, .. } => {
...
impl<'tx> StateStoreReadTransaction for ValidatorNodeStateStoreReadTransaction<'tx> {
    type Addr = PeerAddress;

    fn last_sent_vote_get(
        &self,
    ) -> Result<tari_dan_storage::consensus_models::LastSentVote, tari_dan_storage::StorageError> {
        match self {
    ...
    fn last_executed_get(
        &self,
    ) -> Result<tari_dan_storage::consensus_models::LastExecuted, tari_dan_storage::StorageError> {
        match self {
    ...
    fn last_proposed_get(
        &self,
    ) -> Result<tari_dan_storage::consensus_models::LastProposed, tari_dan_storage::StorageError> {
        match self {
    ...
    fn locked_block_get(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<tari_dan_storage::consensus_models::LockedBlock, tari_dan_storage::StorageError> {
        match self {
    ...
    fn leaf_block_get(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<tari_dan_storage::consensus_models::LeafBlock, tari_dan_storage::StorageError> {
        match self {
    ...
    fn high_qc_get(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<tari_dan_storage::consensus_models::HighQc, tari_dan_storage::StorageError> {
        match self {
    ...
    fn foreign_proposals_has_unconfirmed(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<bool, tari_dan_storage::StorageError> {
        match self {
    ...
    fn foreign_proposal_get_all_pending(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.foreign_proposal_get_all_pending(from_block_id, to_block_id)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.foreign_proposal_get_all_pending(from_block_id, to_block_id)
            },
        }
    }

    fn foreign_send_counters_get(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<tari_dan_storage::consensus_models::ForeignSendCounters, tari_dan_storage::StorageError> {
        match self {
    ...
    fn foreign_receive_counters_get(
        &self,
    ) -> Result<tari_dan_storage::consensus_models::ForeignReceiveCounters, tari_dan_storage::StorageError> {
        match self {
    ...
    ) -> Result<tari_dan_storage::consensus_models::TransactionRecord, tari_dan_storage::StorageError> {
        match self {
    ...
    fn transactions_get_paginated(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transactions_get_paginated(limit, offset, asc_desc_created_at)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.transactions_get_paginated(limit, offset, asc_desc_created_at)
            },
    ...
    fn blocks_get(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<tari_dan_storage::consensus_models::Block, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_all_ids_by_height(
        &self,
        epoch: tari_dan_common_types::Epoch,
        height: tari_dan_common_types::NodeHeight,
    ) -> Result<Vec<tari_dan_storage::consensus_models::BlockId>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_genesis_for_epoch(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<tari_dan_storage::consensus_models::Block, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_all_between(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => tx.blocks_get_all_between(
                epoch,
                shard_group,
                start_block_height,
                end_block_height,
            ...
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => tx.blocks_get_all_between(
                epoch,
                shard_group,
                start_block_height,
                end_block_height,
    ...
    fn blocks_is_ancestor(
        &self,
        descendant: &tari_dan_storage::consensus_models::BlockId,
        ancestor: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<bool, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_all_by_parent(
        &self,
        parent: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<Vec<tari_dan_storage::consensus_models::Block>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_pending_transactions(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<Vec<tari_transaction::TransactionId>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn blocks_get_any_with_epoch_range(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.blocks_get_any_with_epoch_range(epoch_range, validator_public_key)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.blocks_get_any_with_epoch_range(epoch_range, validator_public_key)
            },
    ...
    fn block_diffs_get(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<tari_dan_storage::consensus_models::BlockDiff, tari_dan_storage::StorageError> {
        match self {
    ...
    fn quorum_certificates_get(
        &self,
        qc_id: &tari_dan_storage::consensus_models::QcId,
    ) -> Result<tari_dan_storage::consensus_models::QuorumCertificate, tari_dan_storage::StorageError> {
        match self {
    ...
    fn transaction_pool_get_for_blocks(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transaction_pool_get_for_blocks(from_block_id, to_block_id, transaction_id)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.transaction_pool_get_for_blocks(from_block_id, to_block_id, transaction_id)
            },
    ...
    fn transaction_pool_get_many_ready(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transaction_pool_get_many_ready(max_txs, block_id)
            },
    ...
    fn transaction_pool_count(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transaction_pool_count(stage, is_ready, confirmed_stage, skip_lock_conflicted)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.transaction_pool_count(stage, is_ready, confirmed_stage, skip_lock_conflicted)
            },
    ...
    fn transactions_fetch_involved_shards(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transactions_fetch_involved_shards(transaction_ids)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.transactions_fetch_involved_shards(transaction_ids)
            },
    ...
    fn votes_get_by_block_and_sender(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.votes_get_by_block_and_sender(block_id, sender_leaf_hash)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.votes_get_by_block_and_sender(block_id, sender_leaf_hash)
            },
    ...
    fn votes_get_for_block(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<Vec<tari_dan_storage::consensus_models::Vote>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_get(
        &self,
        address: &tari_dan_common_types::SubstateAddress,
    ) -> Result<tari_dan_storage::consensus_models::SubstateRecord, tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_get_max_version_for_substate(
        &self,
        substate_id: &tari_engine_types::substate::SubstateId,
    ) -> Result<(u32, bool), tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
    ...
    fn substates_exists_for_transaction(
        &self,
        transaction_id: &tari_transaction::TransactionId,
    ) -> Result<bool, tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_get_n_after(
        &self,
        n: usize,
        after: &tari_dan_common_types::SubstateAddress,
    ) -> Result<Vec<tari_dan_storage::consensus_models::SubstateRecord>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn substates_get_many_within_range(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.substates_get_many_within_range(start, end, exclude_shards)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.substates_get_many_within_range(start, end, exclude_shards)
            },
    ...
    fn substates_get_many_by_destroyed_transaction(
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.substates_get_many_by_destroyed_transaction(tx_id)
            },
    ...
    fn substate_locks_get_latest_for_substate(
        &self,
        substate_id: &tari_engine_types::substate::SubstateId,
    ) -> Result<tari_dan_storage::consensus_models::SubstateLock, tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.substate_locks_get_latest_for_substate(substate_id)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.substate_locks_get_latest_for_substate(substate_id)
            },
    ...
    fn state_transitions_get_last_id(
        &self,
        shard: tari_dan_common_types::shard::Shard,
    ) -> Result<tari_dan_storage::consensus_models::StateTransitionId, tari_dan_storage::StorageError> {
        match self {
    ...
    fn state_tree_nodes_get(
        ...
        shard: tari_dan_common_types::shard::Shard,
        key: &NodeKey,
    ) -> Result<Node<Version>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn state_tree_versions_get_latest(
        &self,
        shard: tari_dan_common_types::shard::Shard,
    ) -> Result<Option<Version>, tari_dan_storage::StorageError> {
        match self {
    ...
    fn epoch_checkpoint_get(
        &self,
        epoch: tari_dan_common_types::Epoch,
    ) -> Result<tari_dan_storage::consensus_models::EpochCheckpoint, tari_dan_storage::StorageError> {
        match self {
    ...
    fn burnt_utxos_get(
        &self,
        commitment: &UnclaimedConfidentialOutputAddress,
    ) -> Result<tari_dan_storage::consensus_models::BurntUtxo, tari_dan_storage::StorageError> {
        match self {
    ...
    fn foreign_parked_blocks_exists(
        &self,
        block_id: &tari_dan_storage::consensus_models::BlockId,
    ) -> Result<bool, tari_dan_storage::StorageError> {
        match self {
    ...
    fn transaction_pool_has_pending_state_updates(
        &self,
        block_id: &BlockId,
    ) -> Result<bool, tari_dan_storage::StorageError> {
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => {
                tx.transaction_pool_has_pending_state_updates(block_id)
            },
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => {
                tx.transaction_pool_has_pending_state_updates(block_id)
            },
    ...
    fn substate_locks_has_any_write_locks_for_substates<
        'a,
        I: IntoIterator<Item = &'a tari_engine_types::substate::SubstateId>,
    >(
        &self,
        ...
        match self {
            ValidatorNodeStateStoreReadTransaction::Rocksdb(tx) => tx.substate_locks_has_any_write_locks_for_substates(
                exclude_transaction_id,
                substate_ids,
                exclude_local_only,
            ),
            ValidatorNodeStateStoreReadTransaction::Sqlite(tx) => tx.substate_locks_has_any_write_locks_for_substates(
                exclude_transaction_id,
                substate_ids,
                exclude_local_only,
            ),
    ...
    fn foreign_substate_pledges_get_write_pledges_to_transaction<
        'a,
        I: IntoIterator<Item = &'a tari_engine_types::substate::SubstateId>,
    >(
        &self,
