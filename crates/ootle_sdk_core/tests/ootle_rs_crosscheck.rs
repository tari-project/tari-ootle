...
//! **identical** transaction (deterministic fields) as the core's `build_and_encode_public_transfer`.
//!
...
//! and the **same** `tari_bor` encoder that the core calls. So a core-vs-`ootle-rs` match on the
//! decoded deterministic fields validates **orchestration parity** — the instruction sequence, the
//! fee-instruction placement, the input set/ordering, and the bucket-label convention — **NOT** that
//! two independent CBOR encoders agree (they share `tari_bor`). The truly-independent encoder drift
//! check is the TS/Python re-ports.
...
//! `ootle-rs`'s exact `public_transfer` recipe verbatim, and seals it through the core's seal seam
//! (`ootle_sdk_core::tx::seal_public_transfer`). The recipe below is a **hand-copied snapshot** of
...
//!
//! The seal samples a random Schnorr nonce, so `transaction_id` (which chains the signatures) and the
//! raw encoded bytes move across runs. So both the core (when it generated the fixture) and this
//! cross-check compare the sealed transaction **semantically**: decode + re-verify every signature,
//! null the byte-unstable Schnorr scalars, and compare the deterministic decoded fields (instructions,
//! inputs, signer public keys). That is exactly the orchestration this cross-check guards.
...
use harness::{Fixture, OP_BUILD_AND_ENCODE_PUBLIC_TRANSFER, canonicalize_json, fixtures_dir};
use ootle_sdk_core::{
...
/// The orchestration-parity assertion for one committed vector: the shared-builder path must seal to
/// the **identical** transaction (deterministic decoded fields) as the fixture's generator-owned
/// semantic `expected`.
fn assert_crosscheck(rel: &str) {
    ...
    // 2. Seal it (random nonce), via the core's seal seam — the same seal leaf `ootle-rs` resolves to.
    let (transaction, _id) = seal_public_transfer(unsigned, &keys.to_core()).expect("seal succeeds");
    assert!(
        transaction.verify_all_signatures(),
        "cross-check `{rel}`: sealed signatures must verify"
    );
    let encoded_hex = hex::encode(bor_encode(transaction).expect("BOR encode succeeds"));

    // 3. Semantic parity against the generator-owned expected: decode + re-verify + null the byte-unstable Schnorr
    //    scalars, then compare the deterministic decoded fields (instructions, inputs, signer public keys). The
    //    signatures are random on both sides, so only the deterministic envelope is compared — which is exactly the
    //    orchestration this cross-check guards.
    let actual = canonicalize_json(
        decode_and_canonicalize_sealed_transfer(network, &encoded_hex).expect("decode + verify sealed bytes"),
    );
    let expected = canonicalize_json(
        fixture
            .expected
            .sealed_transaction_semantic
            .clone()
            .expect("fixture has expected.sealed_transaction_semantic"),
    );
    assert_eq!(
        actual,
        expected,
        "\n*** ORCHESTRATION-PARITY MISMATCH: sealed transaction (deterministic fields) ***\n fixture: {}\nexpected \
         (core): {}\n  actual (shared builder): {}\n",
        fixture.name,
...
fn crosscheck_detects_divergence() {
    ...
    assert_ne!(
        actual, expected,
        "a perturbed amount must NOT match the committed transaction — else the cross-check proves nothing"
    );
}
