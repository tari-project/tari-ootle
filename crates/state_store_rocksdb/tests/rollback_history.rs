...
use tari_ootle_storage::{
    ...
    consensus_models::RollbackHistoryEntry,
};
...
fn insert_and_list_chronological() {
    ...
    db.with_write_tx(|tx| tx.rollback_history_insert(&later)).unwrap();
    db.with_write_tx(|tx| tx.rollback_history_insert(&earlier)).unwrap();

    let listed = db.with_read_tx(|tx| tx.rollback_history_list()).unwrap();
...
fn two_inserts_same_second_different_epoch_both_survive() {
    ...
    db.with_write_tx(|tx| tx.rollback_history_insert(&a)).unwrap();
    db.with_write_tx(|tx| tx.rollback_history_insert(&b)).unwrap();
...
fn shard_group_and_metadata_round_trip() {
    ...
    db.with_write_tx(|tx| tx.rollback_history_insert(&entry)).unwrap();
    let listed = db.with_read_tx(|tx| tx.rollback_history_list()).unwrap();
