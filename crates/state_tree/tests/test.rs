...
#[test]
fn hash_differs_when_putting_items_at_a_different_version() {
    let ch_1 = change(1, Some(30));
    let ch_2 = change(2, Some(31));
    let ch_3 = change(3, Some(32));
    let ch_4 = change(4, Some(33));
    let ch_5 = change(5, Some(34));
    let ch_6 = change(6, Some(35));

    let mut tester_1 = HashTreeTester::new_empty();
    tester_1.put_changes_at_version(1, vec![ch_1.clone()]);
    tester_1.put_changes_at_version(1, vec![ch_2.clone()]);
    tester_1.put_changes_at_version(1, vec![ch_3.clone()]);
    tester_1.put_changes_at_version(1, vec![ch_4.clone()]);
    tester_1.put_changes_at_version(1, vec![ch_5.clone()]);
    let hash_1 = tester_1.put_changes_at_version(1, vec![ch_6.clone()]);

    let mut tester_2 = HashTreeTester::new_empty();
    tester_2.put_changes_at_version(1, vec![ch_1, ch_2, ch_3]);
    let hash_2 = tester_2.put_changes_at_version(2, vec![ch_4, ch_5, ch_6]);
...
fn proofs() {
    ...
    let tree = tester.new_state_tree();
