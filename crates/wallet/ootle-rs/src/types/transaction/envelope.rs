...
/// The raw transaction bytes. Encoded as CBOR. For human-readable formats (e.g. JSON) CBOR bytes are encoded as base64
...
impl SendRawTransactionRequest {
    ...
    pub fn decode(&self) -> Result<Signed<OotleTransferTransaction>, BorError> {
        tari_bor::decode(&self.0)
    }

    pub fn encode(tx: &Signed<OotleTransferTransaction>) -> Result<Self, BorError> {
...
pub struct OotleTransferTransaction {
