...
pub struct Block {
    ...
    pub commit_qc_id: Option<String>,
...
impl Block {
    pub fn try_convert(self, qc: sql_models::QuorumCertificate) -> Result<consensus_models::Block, StorageError> {
        ...
        Ok(consensus_models::Block::load(
            ...
            self.commit_qc_id.as_ref().map(deserialize_hex_try_from).transpose()?,
            self.justify_qc_id.as_ref().map(deserialize_hex_try_from).transpose()?,
