...
impl<'a, TStore: WalletStore> KeyManagerApi<'a, TStore> {
    ...
    pub fn get_public_key(&self, branch: &str, key_index: Option<u64>) -> Result<RistrettoPublicKeyBytes, KeyManagerApiError> {
        let (_, key) = self.get_key_or_active(branch, key_index)?;
        Ok(RistrettoPublicKeyBytes::from_secret_key(&key.key))
    }
