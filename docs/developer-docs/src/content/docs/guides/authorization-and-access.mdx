...
impl MyComponent {
    pub fn create_with_restricted_resource(
        minter_badge: ResourceAddress,
    ) -> ResourceAddress {
        let resource_address = ResourceBuilder::public_fungible()
            .with_token_symbol("RTK")
            .divisibility(10) // Set to 10 decimal places
            // Mint an initial supply of 100 million tokens
            .initial_supply(amount!["1000000000000000000000"])
            // Only minter_address can mint more tokens
            .mintable(AccessRule::Restricted(vec![minter_address.into()]))
