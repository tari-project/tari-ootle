...
Now for the prize. We want to give out a unique NFT for each round.
For this, we'll create a new non-fungible resource that uniquely represents a prize for our game.
Later we'll use this resource to mint a prize for each round.
...
<Aside type="note">
  By default, only the resource owner can mint new tokens for a resource. You can change this, but for this example,
  this is exactly what we want.
</Aside>

<Aside type="tip">
  A vault can only hold one and only one resource. If you or anyone else later try to deposit a different resource in
...
pub fn start_game(&mut self, prize: NonFungibleId) {
  ...
  // Mint the prize. Each NFT has immutable (cannot be changed) data and mutable (holder can change) data.
