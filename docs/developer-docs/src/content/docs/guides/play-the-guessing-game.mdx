...
description: Use ootle-rs to write a Rust app that deploys and plays the guessing game template on Tari Ootle.
---
...
// Generate a new key pair using the in-memory provider. In a real app, you may use some other key providers that store secrets securely.
...
// Build a transaction that calls the faucet to deposit 10 XTR into our account.
...
let outcome = pending.watch().await?; // wait for the transaction to be finalized
...
let unsigned_tx = TransactionBuilder::new(provider.network())
    ...
    // call_function invokes a constructor (a free function) on the template
...
// The prize resource that the constructor created (everything except XTR is the game resource)
...
// Sign with the player's wallet — this is what authenticates them in the template
let tx = TransactionRequest::default()
    .with_transaction(unsigned_tx)
...
Use cargo-generate to download the full crate:
