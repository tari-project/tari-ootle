...
Every transaction must pay fees to validators. Fee instructions are a separate sequence that runs **before** the main instructions. They lock up a maximum amount of XTR (the native token) from a specified account or bucket.
...
let transaction = Transaction::builder(network)
    ...
    // Withdraw 100 tokens of `my_resource` into a Bucket from the sender's account
...
let transaction = Transaction::builder(network)
    ...
    .call_function(template_address, "new_with_address", args![Workspace("my_addr")])
    .build_and_seal(&secret_key);
```
...
```rust
let transaction = Transaction::builder(network)
    // Lock up at most 1000 XTR for fees; refund the rest
...
let transaction = Transaction::builder(network)
    .with_fee_instructions_builder(|builder| {
        builder
            ...
            .call_method(my_account, "deposit", args![Workspace("faucet_coins")])
...
let template_address: TemplateAddress = /* the address of your deployed template */;
...
#[test]
fn test_my_template() {
  let mut test = TemplateTest::new(env!("CARGO_MANIFEST_DIR"), ["path/to/my_template"]);
