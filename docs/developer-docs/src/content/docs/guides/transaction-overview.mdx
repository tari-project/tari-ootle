...
| `PublishTemplate(blob_index)`                                          | Publishes a new WASM template referenced from the transaction's [blob list](#blobs) |
...
- `InstructionArg::Blob(idx)` passes the blob's bytes as a function/method argument — same shape
  as a `Literal` argument from the template's perspective, just carried out-of-band.

### Adding and referencing blobs

The `TransactionBuilder` mirrors the workspace-item naming pattern. Register a blob under a
...
For the common one-shot case where you only want to publish a template and never read the same
...
`add_blob` panics if the transaction already has 256 blobs (the `BlobIndex` cap). Use
`add_blob_checked` if you need the fallible variant.

### Blob vs. Literal: which to use

Both `Literal` and `Blob` arguments arrive at the template as the same decoded value — there is
...
Manifest text can reference blobs via the `blob!(name)` macro. The actual bytes are supplied
...
Repeated references to the same name reuse the same `BlobIndex` — you don't need to add the
